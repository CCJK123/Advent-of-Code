def day_18(input_str):
    # Initial setup
    import copy, itertools
    input_array = input_str.split("\n")
    
    def setup_grid():
        grid_now = [[None]*(len(input_array[0])+2) for i in range(len(input_array)+2)]
        ...
        for row_no, row in enumerate(grid_now[1:-1], 1):
            for col_no, light in enumerate(row[1:-1], 1):
                ...
                for offset in itertools.product((-1, 0, 1), repeat=2):
                    if offset != (0, 0):
                        neighbours.append(grid_now[row_no+offset[0]][col_no+offset[1]])
    ...
    grid_now = setup_grid()
    def on_corners(grid):
        for row_no, col_no in itertools.product((1, len(input_array)), (1, len(input_array[0]))):
