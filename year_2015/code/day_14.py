def day_14(input_str):
    ...
    for data in input_array:
        distances[data[0]] = data[1]*(data[2]*(duration//(data[2]+data[3]))+min(data[2], duration%(data[2]+data[3])))
    ...
    for timestep in range(duration):
        for data in input_array:
            distances[data[0]] += (data[1] if (timestep%(data[2]+data[3])<data[2]) else 0)
        for name in [name for name, distance in distances.items() if distance==max(distances.values())]:
            points[name] += 1
