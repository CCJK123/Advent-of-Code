            ...
            if command[0][3:] == "..":
                ...
                for size in directory_sizes["/".join(current_directory_path)].copy():
                    if type(size) == str:  # i.e. a temp directory pointer
                        directory_sizes["/".join(current_directory_path)].remove(size)
                        directory_sizes["/".join(current_directory_path)].append(
                            directory_sizes[
                                "/".join(current_directory_path.copy() + [size])
                            ]
                        )
                directory_sizes["/".join(current_directory_path)] = sum(
    ...
    while current_directory_path != []:
        ...
        for size in directory_sizes["/".join(current_directory_path)].copy():
            if type(size) == str:  # i.e. a temp directory pointer
                directory_sizes["/".join(current_directory_path)].remove(size)
                directory_sizes["/".join(current_directory_path)].append(
                    directory_sizes["/".join(current_directory_path.copy() + [size])]
                )
        directory_sizes["/".join(current_directory_path)] = sum(
            directory_sizes["/".join(current_directory_path)]
        )
