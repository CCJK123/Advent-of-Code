def run(input_str):
    ...
    for ingredient_amts in (
        i
        for i in itertools.product(range(101), repeat=len(input_array))
        if sum(i) == 100
    ):
        score = 1
        for metric_index in range(1, 5):
            score *= max(
                ...
                sum(
                    (
                        ingredient_amts[i] * input_array[i][metric_index]
        ...
        if (
            sum(
                (
                    ingredient_amts[i] * input_array[i][5]
                    for i in range(len(input_array))
                )
            )
            == 500
