def day_11(input_str):
    ...
    for i in range(len(input_array)):
        input_array[i] = {
            ...
            "operation": input_array[i][2][11:].replace("old", "new").replace("new", "item_worry_level"),
            "test": [
        ...
        for monkey_no in range(len(input_array)):
            for item_worry_level in monkeys[monkey_no]["items"]:
                ...
                if item_worry_level % monkeys[monkey_no]["test"][0] == 0:
                    monkeys[monkeys[monkey_no]["test"][1][True]
                            ]["items"].append(item_worry_level)
                else:
                    monkeys[monkeys[monkey_no]["test"][1][False]
                            ]["items"].append(item_worry_level)
            # Clear items
    ...
    divisibility_tests_lcm = lcm(
        *[monkeys[i]["test"][0] for i in range(len(monkeys))])
