    ...
    for i in range(len(input_array)):
        for j in range(len(input_array[i])):
            if input_array[i][j] <= 1:
                ...
                # # Not top row
                if i != 0:
                    if input_array[i-1][j] <= 1 or input_array[i-1][j] - input_array[i][j] > 1:
                        is_viable[0] = False
                # # Not bottom row
                if i != len(input_array) - 1:
                    if input_array[i+1][j] <= 1 or input_array[i+1][j] - input_array[i][j] > 1:
                ...
                # # Not left column
                if j != 0:
                    if input_array[i][j-1] <= 1 or input_array[i][j-1] - input_array[i][j] > 1:
                        is_viable[2] = False
                # # Not right column
                if j != len(input_array[i]) - 1:
                    if input_array[i][j+1] <= 1 or input_array[i][j+1] - input_array[i][j] > 1:
                ...
                if True in is_viable:
                    try:
                        if min_path_length != None:
                            min_path_length = min(
                                min_path_length, nx.dijkstra_path_length(pathways, (i, j), end))
                        else:
                            min_path_length = \
