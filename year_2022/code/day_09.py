def day_09(input_str):
    ...
    has_visited_y_x = np.full(
        (len(range(*vertical_max_min[::-1]))+1, len(range(*horizontal_max_min[::-1]))+1), False)
    rope_coords_y_x = np.array(
        [[-vertical_max_min[1], -horizontal_max_min[1]]]*10
    )
    ...
    for motion in input_array:
        for distance_moved in range(1, motion[1]+1):
            ...
            for rope_part_num in range(10-1):
                if np.all(np.isin(abs(rope_coords_y_x[rope_part_num]-rope_coords_y_x[rope_part_num+1]), (0, 1))):
                    # Lead rope part is adjacent to follower rope part, don't move follower
                    pass
                else:
                    # Shift follower rope part accordingly
                    y_coord_diff = rope_coords_y_x[rope_part_num][0] - \
                        rope_coords_y_x[rope_part_num+1][0]
                    x_coord_diff = rope_coords_y_x[rope_part_num][1] - \
                    ...
                    if abs(y_coord_diff) > abs(x_coord_diff):
                        ...
                        if x_coord_diff > 0:
                            rope_coords_y_x[rope_part_num+1] = \
                                rope_coords_y_x[rope_part_num] + (0, -1)
                        else:
                            rope_coords_y_x[rope_part_num+1] = \
                                rope_coords_y_x[rope_part_num] + (0, 1)
